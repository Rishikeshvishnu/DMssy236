...
public:
    ...
    WorldInfo(ros::NodeHandle& nh)
    {
        ...
        client_map_generator_ = nh.serviceClient<final_project::UpdateObjectList>(srv_update_obj_name_);
  ...
  void topic_callback(const gazebo_msgs::ModelStates::ConstPtr& msg)
  {
   ...
   auto it = std::find( msg->name.begin(),  msg->name.end(), "tiago");
    ...
    for (int i = 0; i < msg->name.size(); i++)
    {
        ...
        double dx = obj_pose.position.x - tiago_pose.position.x;  // (or) tiago_pose.position.x - obj_pose.position.x
        //TODO: obtain the dy distance between the robot and the objects
        double dy = obj_pose.position.y - tiago_pose.position.y;  // (or) tiago_pose.position.y - obj_pose.position.y

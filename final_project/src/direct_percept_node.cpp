...
/**
  ...
  void topic_callback(const gazebo_msgs::ModelStates::ConstPtr& msg)
  {
    ...
    for (int i = 0; i < msg->name.size(); i++)
    {
        ...
        tf.setOrigin(tf2::Vector3(pose.position.x, pose.position.y, pose.position.z)); // Point position
        tf.setRotation(tf2::Quaternion(pose.orientation.x, pose.orientation.y, pose.orientation.z, pose.orientation.w)); // Quaternion orientation

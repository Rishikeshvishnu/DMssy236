...
/**
  ...
  void topic_callback(const gazebo_msgs::ModelStates::ConstPtr& msg)
  {
    ...
    auto it = std::find(msg->name.begin(), msg->name.end(),"tiago"); //msg->name.end(); 
    if (it != msg->name.end()) 
    {
        // Calculate the index
    ...
    for (int i = 0; i < msg->name.size(); i++)
    {
        ...
        geometry_msgs::Pose obj_pose= msg->pose[i]; //geometry_msgs::Pose();
        ...
        if (d<1.1)
        {
            ...
            std::string s= msg->name[i];//"something";
            ...
            if (it == v_seen_obj_.end()) {
                ...
                srv.request.object_name= s;//"object";
                //TODO: send the pose of the seen object to the service (0.25 pts)
                srv.request.object_pose= msg->pose[i];//geometry_msgs::Pose();
